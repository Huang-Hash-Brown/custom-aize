...
const createDatabaseAndConfigure = async () => {
  ...
  const creationOutput = executeCommand(`pnpm wrangler d1 create ${dbName}`)
  ...
  if (creationOutput === undefined || typeof creationOutput !== 'string') {
    ...
    const dbInfoOutput = executeCommand(`pnpm wrangler d1 info ${dbName}`)
...
const promptForGoogleClientCredentials = async () => {
...
const updateDevVarsWithSecret = async () => {
...
const runDatabaseMigrations = async (dbName: string) => {
  const setupMigrationSpinner = spinner()
  setupMigrationSpinner.start('Generating setup migration...')
  executeCommand('pnpm drizzle-kit generate --name setup')
  ...
  executeCommand(`pnpm wrangler d1 migrations apply ${dbName}`)
  ...
  executeCommand(`pnpm wrangler d1 migrations apply ${dbName} --remote`)
...
const main = async () => {
  try {
    ...
    console.log('\x1b[33mRunning pnpm run dev command...\x1b[0m')
